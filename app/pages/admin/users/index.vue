    ...
    >
      ...
      <template #actions="{ row }">
        <div class="flex flex-wrap items-center justify-end gap-2">
          ...
          <NuxtLink :to="`/admin/users/${row.id}/wallet`">
      ...
      <div v-else class="space-y-4">
        <div class="grid gap-3 sm:grid-cols-2">
          ...
          <div class="rounded-2xl border border-slate-200 bg-white p-4">
            ...
            <div class="mt-4 rounded-2xl bg-slate-950 px-4 py-4 text-white">
              <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-300">Wallet balance</p>
              <p class="mt-2 text-2xl font-semibold">{{ formatUSD(viewUser.walletBalance) }}</p>
              <div class="mt-3 grid grid-cols-2 gap-2 text-[11px] text-slate-300">
                <p><span class="font-semibold text-white">Credits</span> {{ formatUSD(viewUser.totalCredits) }}</p>
                <p class="text-right"><span class="font-semibold text-white">Debits</span> {{ formatUSD(viewUser.totalDebits) }}</p>
                <p><span class="font-semibold text-white">Orders</span> {{ viewUser.ordersCount }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3">
              <p class="text-xs font-semibold text-slate-900">Recent wallet transactions</p>
              <p class="text-[11px] text-slate-500">Last 5</p>
            </div>
            <div v-if="viewUser.recentWallet.length === 0" class="p-4 text-xs text-slate-600">No transactions.</div>
            <div v-else class="divide-y divide-slate-200">
              <div v-for="tx in viewUser.recentWallet" :key="tx.id" class="px-4 py-3 text-xs">
                <div class="flex items-start justify-between gap-2">
                  <div class="min-w-0">
                    <p class="font-medium text-slate-900">{{ tx.description }}</p>
                    <p class="mt-1 text-[11px] text-slate-500">{{ tx.date }}</p>
                  </div>
                  <p class="font-semibold" :class="tx.type === 'credit' ? 'text-emerald-700' : 'text-red-700'">
      ...
      <div v-else class="space-y-4">
        ...
        <div class="rounded-2xl bg-slate-950 px-4 py-4 text-white">
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 text-[11px] text-slate-300">
            ...
            <p class="text-right"><span class="font-semibold text-white">Debits</span> {{ formatUSD(viewUser.totalDebits) }}</p>
            <p><span class="font-semibold text-white">Orders</span> {{ viewUser.ordersCount }}</p>
            <p class="text-right"><span class="font-semibold text-white">Joined</span> {{ viewUser.joinedAt }}</p>
          </div>
        </div>

        <div class="rounded-2xl border border-slate-200 bg-white/70 p-4">
          <p class="text-xs font-semibold text-slate-900">Recent wallet transactions</p>
          <div v-if="viewUser.recentWallet.length === 0" class="mt-3 text-xs text-slate-600">No transactions.</div>
          <div v-else class="mt-3 space-y-2">
            <div v-for="tx in viewUser.recentWallet" :key="tx.id" class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs">
              <div class="flex items-start justify-between gap-2">
                <div class="min-w-0">
                  <p class="font-medium text-slate-900">{{ tx.description }}</p>
                  <p class="mt-1 text-[11px] text-slate-500">{{ tx.date }}</p>
...
const viewUser = ref({
  ...
  ordersCount: 0,
  joinedAt: "—",
  recentWallet: [] as Array<{ id: string; type: "credit" | "debit"; amount: number; description: string; date: string }>,
...
const loadUserDetails = async (uid: string) => {
  ...
  try {
    ...
    const walletTxs = Array.isArray(data?.walletTransactions) ? (data.walletTransactions as WalletTransaction[]) : []
    const totalCredits = walletTxs.filter((t) => t.type === "credit").reduce((a, t) => a + Number(t.amount ?? 0), 0)
    const totalDebits = walletTxs.filter((t) => t.type === "debit").reduce((a, t) => a + Number(t.amount ?? 0), 0)

    const recentWallet = [...walletTxs]
      .sort((a: any, b: any) => {
